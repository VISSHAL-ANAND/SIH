...
def test_open_meteo_provider_converts_current_and_wind_to_components(monkeypatch):
    ...
    def fake_get(url, params, timeout):
        ...
        if "marine-api" in url:
            return FakeResponse({
                "hourly": {
                    "time": ["2026-06-21T22:00", "2026-06-21T23:00"],
    ...
    rows = provider.get_observations(10.0, 80.0, datetime(2026, 6, 21, 23, 0, tzinfo=timezone.utc))

    assert len(rows) == 2
    assert rows[0]["source"] == "Open-Meteo Marine + Forecast API"
    assert rows[0]["quality"] == "MODELLED"
    # 3.6 km/h = 1 m/s; current at 90 degrees flows east.
    assert abs(rows[0]["current_u_mps"] - 1.0) < 1e-6
    assert abs(rows[0]["current_v_mps"]) < 1e-6
    # Meteorological 90 degree wind comes from east, so it flows west.
