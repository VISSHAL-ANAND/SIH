...
=======================================================
Government-style FastAPI service for real SAR oil-spill analysis,
AIS candidate correlation, environmental drift evidence, and
operator-reviewed response/report drafts.

Runtime contract:
  POST /api/process-sar       — SAR upload + real inference pipeline
...
canonical API. Missing external evidence is returned explicitly as unavailable.
"""
...
def _incident_spill_anchor(
    ...
    location = next(
        (c.get("geolocation") for c in components if c.get("geolocation")),
        None,
    )
...
def _attach_live_environment(result: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if lat is None or lon is None or not timestamp:
        result["environmental"] = {
            ...
            "observation_count": 0,
            "reason": (
                "A real georeferenced spill location and detection timestamp are "
                "required before environmental retrieval."
            ),
        }
        return result
    ...
    try:
        ...
        rows = OpenMeteoEnvironmentalProvider().get_observations(
            lat, lon, detection_time, window_hours=3
        )
...
async def process_sar(
    ...
    """Run the canonical real-only SAR investigation pipeline."""
    ...
    try:
        ...
        incident_id = (
            result.get("incident", {}).get("incident_id")
            or f"IMW-{uuid.uuid4().hex[:8].upper()}-2026"
        )
        ...
        raise HTTPException(
            status_code=503,
            detail=f"IMW real pipeline unavailable: {exc}",
        ) from exc
    finally:
        try:
...
async def analyze_traffic(request: AnalyzeTrafficRequest) -> Dict[str, Any]:
    ...
    if not ais_path.exists():
        return {
            ...
            "surrounding_traffic": [],
            "attribution_reason": (
                "Configured AIS source is unavailable; no vessel attribution is made."
            ),
        }
    ...
    if request.capture_time:
        capture_time = datetime.fromisoformat(
            request.capture_time.replace("Z", "+00:00")
        ).replace(tzinfo=None)
    else:
        capture_time = datetime.now(timezone.utc).replace(tzinfo=None)
